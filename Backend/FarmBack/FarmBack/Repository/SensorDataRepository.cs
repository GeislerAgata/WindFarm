...
public class SensorDataRepository : ISensorDataRepository
{
    ...
    public List<SensorData> GetSensorsData(string filters, string sortBy, string order, int limit)
    {
        ...
        if (limit != 0)
        {
            return _sensorDataCollection.Find(filter).Limit(limit).ToList();
        }
    ...
    public SensorStats GetSensorStats(int sensorId)
    {
        var filter = Builders<SensorData>.Filter.Eq(x => x.SensorId, sensorId);
        ...
        {
            double sum = 0;
            double lastValue = sortedData[0].Value;
            ...
            double avg = sum / sortedData.Count;
